...
void ImGui::RenderTextClipped( const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, ImGuiAlign align, const ImVec2* clip_min, const ImVec2* clip_max )
{
	...
	if( align & ImGuiAlign_Center )
		pos.x = ImMax( pos.x, ( pos.x + pos_max.x - text_size.x ) * 0.5f );
	else if( align & ImGuiAlign_Right )
		pos.x = ImMax( pos.x, pos_max.x - text_size.x );
	if( align & ImGuiAlign_VCenter )
		pos.y = ImMax( pos.y, ( pos.y + pos_max.y - text_size.y ) * 0.5f );
...
bool ImGui::Begin( const char* name, bool* p_opened, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags )
{
	...
	if( first_begin_of_the_frame )
	{
		...
		if( !( flags & ImGuiWindowFlags_NoTitleBar ) )
		{
			...
			if( pad_right )
				text_max.x -= g.FontSize + style.ItemInnerSpacing.x;
...
static bool CloseWindowButton( bool* p_opened )
{
	...
	window->DrawList->AddRectFilled( window->Rect().GetTR() + ImVec2( -2.0f - size, 2.0f ), window->Rect().GetTR() + ImVec2( -2.0f, 2.0f + size ), col );
...
namespace ImGuiStb
{
	...
	{
		return ImCharIsSpace( c ) || c == ',' || c == ';' || c == '(' || c == ')' || c == '{' || c == '}' || c == '[' || c == ']' || c == '|';
	}
...
bool ImGui::InputTextEx( const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data )
{
	...
	if( g.ActiveId == id )
	{
		...
		{
			edit_state.OnKeyPressed( is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT | k_mask : STB_TEXTEDIT_K_LEFT | k_mask );
		}
		else if( IsKeyPressedMap( ImGuiKey_RightArrow ) )
		{
			edit_state.OnKeyPressed( is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT | k_mask : STB_TEXTEDIT_K_RIGHT | k_mask );
		}
		...
		else if( is_shortcutkey_only && IsKeyPressedMap( ImGuiKey_Z ) && is_editable )
		{
			edit_state.OnKeyPressed( STB_TEXTEDIT_K_UNDO );
			edit_state.ClearSelection();
		}
		else if( is_shortcutkey_only && IsKeyPressedMap( ImGuiKey_Y ) && is_editable )
		{
			edit_state.OnKeyPressed( STB_TEXTEDIT_K_REDO );
			edit_state.ClearSelection();
		}
		else if( is_shortcutkey_only && IsKeyPressedMap( ImGuiKey_A ) )
		{
			edit_state.SelectAll();
			edit_state.CursorFollow = true;
		}
			...
			static void NodeDrawList( ImDrawList* draw_list, const char* label )
			{
				...
				for( const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount , pcmd++ )
				{
					...
					if( show_clip_rects && ImGui::IsItemHovered() )
					{
						...
						clip_rect.Round();
						overlay_draw_list->AddRect( clip_rect.Min, clip_rect.Max, ImColor( 255, 255, 0 ) );
						vtxs_rect.Round();
						overlay_draw_list->AddRect( vtxs_rect.Min, vtxs_rect.Max, ImColor( 255, 0, 255 ) );
					}
					if( !draw_opened )
						continue;
